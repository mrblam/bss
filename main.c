...
void HAL_STATE_MACHINE_UPDATE_TICK(void) {
        ...
        for(int i=0;i<BP_NUM;i++){
                ...
                if(bp_data[i].inactive_time>BP_INACTIVE_TIMEOUT_mS){
                        ...
                        bp_set_con_state(&bp_data[i],BP_CON_ST_DISCONNECTED);
                        sdo_server_set_state(&bp_data[i].sdo_server,SDO_ST_IDLE);
                }
                bp_sdo_server_check_timeout(&bp_data[i],sys_timestamp);
        }
}
...
static char* json_serialize(const BP_PDO_Data* const p_bp,char* buff){
        ...
        *buff++=',';
        buff=json_from_long(buff,"soc",p_bp->soc);
        *buff++=',';
        buff=json_from_long(buff,"soh",p_bp->soh);
        *buff++=',';
        buff=json_from_long(buff,"state",p_bp->state);
        *buff++=',';
        buff=json_from_long(buff,"status",p_bp->status);
...
static int32_t bp_data_serialize(char* buff){
        ...
        *buff='\0';
        for(int i=0;i<BP_NUM;i++){
                if(bp_get_con_state(&bp_data[i])==BP_CON_ST_AUTHORIZED){
